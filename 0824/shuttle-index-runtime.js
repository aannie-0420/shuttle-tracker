(function (root, factory) {
  ...
  function readScheduleEntries(root) {
    ...
    // 巡迴列不是一般班次列，但16:35是通勤建議可搭乘的首班時間。
    const loopRow = typeof root.querySelector === "function"
      ? root.querySelector("#schedule-small-col .schedule-loop")
      : null;
    const loopStart = loopRow && loopRow.getAttribute("data-loop-start");
    if (/^\d{2}:\d{2}$/.test(loopStart || "")) {
      entries.push({
        type: "small",
        time: loopStart,
        servesA8: true,
  ...
  function isLoopServiceActive(now = new Date()) {
    ...
    return minutes >= 15 * 60 + 35 && minutes <= 18 * 60 + 30;
  }

  function updateScheduleHighlights(root, currentStr, now = new Date()) {
    if (!root || typeof root.querySelectorAll !== "function") return false;
    const scheduleItems = Array.from(root.querySelectorAll(".schedule-item"));
    const loopRows = Array.from(root.querySelectorAll(".schedule-loop"));
    [...scheduleItems, ...loopRows].forEach(row => {
      if (row.classList && typeof row.classList.remove === "function") {
        row.classList.remove("next-shuttle");
      }
    });

    const nowValue = now instanceof Date ? now : new Date(now);
    const active = isLoopServiceActive(nowValue);
    if (active) {
      const loopRow = loopRows[0];
      if (loopRow && loopRow.classList && typeof loopRow.classList.add === "function") {
        loopRow.classList.add("next-shuttle");
      }
      const mediumRows = Array.from(root.querySelectorAll("#schedule-medium-col .schedule-item"))
        .filter(row => {
          const time = row.getAttribute("data-time");
          return /^\d{2}:\d{2}$/.test(time || "") && time >= currentStr;
        })
        .sort((left, right) => left.getAttribute("data-time").localeCompare(right.getAttribute("data-time")));
      if (mediumRows[0] && mediumRows[0].classList && typeof mediumRows[0].classList.add === "function") {
        mediumRows[0].classList.add("next-shuttle");
      }
      return true;
    }

    const futureRows = scheduleItems
      .map(row => ({ row, time: row.getAttribute("data-time") }))
      .filter(item => /^\d{2}:\d{2}$/.test(item.time || "") && item.time >= currentStr)
      .sort((left, right) => left.time.localeCompare(right.time));
    futureRows.slice(0, 2).forEach(item => {
      if (item.row.classList && typeof item.row.classList.add === "function") {
        item.row.classList.add("next-shuttle");
      }
    });
    return false;
  }
